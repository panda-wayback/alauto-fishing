...
- `first_click_trigger/` — 声音开钓（独立 Worker + 会话态）
  - `audio_input.py` — Windows：WASAPI Loopback（输出环回，不按 max_input 拒开）；macOS：虚拟输入；列表带 is_loopback
  ...
  - `paths.py` — 用户库 `audio_template/` + 内置 `assets/audio/*.npy`；长录音 `audio_sessions/`；冻结包 `seed/` 缺则拷到本机 data
