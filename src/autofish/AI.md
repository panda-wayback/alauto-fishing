...
- bobber_anchor 识别器（默认）：先 `FindBobber`；同高由内向外彩色 `endcap_*_full` 定界；整段 ≥20fps（&lt;50ms）；`use_bobber_anchor()` 可显式切回
- find_bobber（独立）：颜色结构候选（红箍+橘羽/亮肚）+ 小 ROI 彩色模板复核（尺度约 0.16～0.64，按估邻近少档）；不定条；宁可 miss 不可错
- 壳订 Frame（画面）+ Pos（读数）。
- Decide 不点鼠标；Act 不决策。
- 禁止依赖 `sim` / `ui` / `algo`。
