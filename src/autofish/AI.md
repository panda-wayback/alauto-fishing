...
- `FirstClickTrigger`：仅 `WAIT` 且冷却已过才听声并更新抬升基线；环缓 20s；开钓页可滚动画波形+命中橙标；立刻进 `FIRST_CLICK`（等待/长按/松开后区间可配置）→ `WAIT_BOBBER`（3s）→ 出漂 `FISHING` → 丢漂满 1s 回 `WAIT`（再冷却 3s）；失败回 `WAIT`；发布 `CAST_SESSION`
- 壳订 Frame（画面）+ Pos（读数）。
- Decide 不点鼠标；Act 不决策。
- 禁止依赖 `sim` / `ui` / `algo`。
