...
- `detect/` — 段3 识别器（bobber_anchor）→ Pos（**不改 mss**）
- `decide/` — 段4 策略；A 开时仅会话 `FISHING` 才发按住意图
...
- `first_click_trigger/` — 声音开钓（独立 Worker + 会话态）
  ...
  - `ring_buffer.py` — 最近 20 秒循环缓冲（开钓页实时波形）
...
- `FirstClickTrigger` — 可挂总线发会话态、订 Pos；模板加载/选区入库；播放试听
...
- Detect：bobber_anchor（先漂后端帽）；识别线程异常不退出；监控重启重置帧序
- Detect 计时：`PosEvent.detect_ms` = 单帧识别耗时；壳显示 + 每 2s 日志（仅近 2s，标条内/全图）
- bobber_anchor：先准确定漂 → 手动/自动锁条 → 有条后只在条内跟漂；入图**等比压入 800×800** 再识、坐标映回；模板档约 0.14～1.0、多试邻近档、**命中锁档**；**无 local**；跟漂 mid≈条长½（夹在 full-nbr 内）；**禁止同帧 mid+full**；mid 丢→隔帧 full-nbr 恢复；邻域**底贴条底、只向上扩**；锁档条内**命中即停**；**监控开关保留程序锁**（ROI 未变）；**有漂无条仍回报漂位**（pos 空）；整段 ≥20fps；`apply_manual_bar`
