...
- 规则（与 [`docs/simulator/algo-test/`](../../simulator/algo-test/) 一致）：
  - 绿区相对 pos **&lt; 50** → 意图为按住（点击）
  - pos **&gt; 90** → 意图为松开
  - 50～90 → 保持当前意图
- 切换约束：按住↔松开两次切换至少间隔 **0.2s**，再加 `U(0, 0.1)` s；未到间隔不改意图。
- 无有效 Pos / 非钓鱼态 → 意图为松开（安全）。
- 可复用 `src/algo` 阈值策略思路；观测用绿区 0～100 与真机 Pos 对齐。
- 不做：mss、CV、键鼠落点；**不**在本段改 50/90 阈值（阈值属策略配置，非运行时自改）。
