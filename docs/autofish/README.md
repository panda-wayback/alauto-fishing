...
- **预览/调试壳**：属 `src/tools/`，**不是**五段之一。**PySide6** 单窗：顶栏控件 + **左监控 + 右条界标记** + 日志。订 Frame（画面）+ Pos（读数）；**监控 / 策略 / 操作** 用勾选独立启停；**策略与操作默认开**。右侧为识别参考图：叠程序条界与手动条界（手动优先覆盖）；**有漂即出参考图**；**不嵌模拟器手玩**。**只要按钮/勾选，不要快捷键**。截屏：直接抓主屏（不挪窗）→ 拖拽手框；**确认框选后自动开监控**。**有存盘手框（及条界）时，下次启动自动载入并开监控**，无需再框选/再标。监控原画面；叠层：有漂画漂点，有条画条框（可区分有漂无条）。顶栏 **「权限」**：macOS 屏幕录制/辅助功能状态与去授权；Windows 截屏探测与管理员状态/提权重启。详见 [`docs/common/permissions/`](../common/permissions/)。
...
6. 调试壳改 PySide6（监控+模拟同窗、按钮化、截屏不最小化）（已完成）。
