...
- **预览/调试壳**：属 `src/tools/`，**不是**五段之一。**PySide6** 单窗：顶栏控件 + 左监控 + 右模拟器 + 日志。订 Frame（画面）+ Pos（读数）；**监控 / 策略 / 操作** 用勾选独立启停；**策略与操作默认开**。模拟器仅手玩嵌右侧；壳禁止把 Act 写入模拟器。**只要按钮/勾选，不要快捷键**。截屏：直接抓主屏（不挪窗）→ 拖拽手框；**确认框选后自动开监控**。监控原画面；叠层仅标命中。顶栏 **「权限」**：macOS 屏幕录制/辅助功能状态与去授权；Windows 截屏探测与管理员状态/提权重启。详见 [`docs/common/permissions/`](../common/permissions/)。
